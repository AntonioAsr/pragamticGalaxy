...
class RoboFile extends \Robo\Tasks {
	...
	public function wordpressSetup( $opts = [
		...
		$this->_exec( "mysql -uroot -h 0.0.0.0 -e 'create user if not exists " . $opts['wp-db-name'] . "'" );
		$this->_exec( "mysql -uroot  -h 0.0.0.0 -e 'create database if not exists " . $opts['wp-db-name'] . "'" );
		$this->_exec( 'mysql -uroot -h 0.0.0.0 -e "grant all privileges on ' . $opts['wp-db-name']
		. ' . * to ' . $opts['wp-db-name'] . "@localhost identified by '" . $opts['wp-db-name'] . "'\"" );
		$this->_exec( "mysql -uroot -h 0.0.0.0 -e 'flush privileges'" );
